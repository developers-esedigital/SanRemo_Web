  ...
  var Editor = function (handler) {
    ...
    this.createLink = function ($editable, linkInfo, options) {
      ...
      if (isTextChanged) {
        ...
        var anchor = rng.insertNode($("<A onclick='window.open("+linkUrl+", '_system', 'location=yes'); return false;'>" + linkText + '</A>')[0]);
        anchors.push(anchor);
        console.log(anchors);
      } else {
      ...
      $.each(anchors, function (idx, anchor) {
        $(anchor).attr('href', linkUrl).attr('onclick', "window.open("+linkUrl+", '_system', 'location=yes'); return false;");
        if (isNewWindow) {
