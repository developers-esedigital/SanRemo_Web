...
class Ban extends CI_Controller {
	...
	public function borrar($id){
		if( $this->ModelBanner->delete($id) ){
			$res = array('code'=>200,'msg'=>'Borrado Exitoso','url'=>base_url().'index.php/ban');
		}else{
	...
	public function edit($id){
		if ($this->input->is_ajax_request()  && $this->input->post('registro') == '1') {
			if( $this->ModelBanner->save($this->input->post())){
				$res = array('code'=>200,'msg'=>'Registro Exitoso','url'=>base_url().'index.php/ban');
			}else{
			...
			$data = array(
				'banner'=>$this->ModelBanner->get($id)
				);
