...
class ModelMarca extends CI_Model {
	function __construct(){
		parent::__construct();
		$this->bd = $this->doctrine->em->getRepository('Marca');
		$this->em = $this->doctrine->em;
	}
	public function get($id){
		$r = $this->bd->findOneBy(array('idmarca'=>$id));
		return $r;
	}
	...
	public function delete($id){
		$exp = $this->bd->findOneBy(array('idmarca'=>$id));
		$this->em->remove($exp);
    	$this->em->flush();
    	return true;
	}
	public function save($data){
		...
		if(isset($data['idmarca'])){
			$oferta = $this->em->getRepository('Marca')->findOneBy(array('idmarca'=>$data['idmarca']));
