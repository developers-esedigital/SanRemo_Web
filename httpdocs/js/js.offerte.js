...
Tools.cargarPrincipal({
	...
	callback: function(rr) {
		for (var i = 0; i < 10; i++) {
			...
			var nombre=Tools.clearStr(JSON.parse(item.idmicrosite.nombre)[validaridioma(idioma)]).toLowerCase();
			console.log(nombre);

			if (nombre == 'su.ma.-immobiliare') {
				// statement
				nombre = "suma-immobiliare";
			...
			elemento = elemento.replace('%src%','//sanremo-on.com/'+validaridioma(idioma)+'/news/offerte/'+nombre+'/'+ JSON.parse(item.url)[validaridioma(idioma)]+'.html');
		...
		$( "#boton" ).click(function() {
			$(".col-sm-6.offer.img-ofert-iz").css('display', 'none');
