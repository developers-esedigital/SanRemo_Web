...
var idioma = Tools.extraerUrls().idioma;
console.log(Tools.extraerCategoria());
var ca = Tools.extraerCategoria();
console.log(ca.categoria);
var bread = '';
switch(idioma){
	case 'ru':
			switch(ca.categoria){
				case 1:
					...
					case 2:
						...
						case 3:
							...
							break;
							case 4:
								bread = 'СВОБОДНОЕ ВРЕМЯ И БЛАГОПОЛУЧИЕ';
								break;
								case 5:
									bread = 'СПЕКТАКЛИ И КИНО ';
									break;
			}
			break;
	case 'fr':
...
console.log(bread);
if (bread == 'Accomodation' || bread == 'Hébergements' || bread == 'УДОБСТВА') {

	$('#pacchetti').css('display', 'block');
	$('#pa').attr('href', 'http://sanremo-on.com/'+idioma+'/news/pacchetti-soggiorno.html');

	// statement
}
$(document).ready(function() {
	$('#pacchetti').on('click',  function(event) {
		event.preventDefault();
		/* Act on the event */
		//alert('click');
		window.location.href = 'http://sanremo-on.com/'+idioma+'/news/offerte-soggiorno.html';
	});
});
