...
var idioM='';
    switch(idioma){
        case 'it':
            idio = "Filtra Risultati";
            idioM="Guarda Tutti";
            break;
            case 'fr':
                idio = "Filtrer les Résultats";
                idioM="Voir Tout";
                break;
                case 'en':
                    idio = "Filter Results";
                    ...
                    break;
                    case 'ru':
                        idio = "результаты фильтрации";
                        idioM="посмотреть все";
                        break;
      
    }
//alert(idio +" "+ idioM);
$('#filtrarR').val(idio);
$('#mostrarTodo').val(idioM);
$('#filtrarRe').val(idio);
$('#servicioButton').val(idioM);
    ...
    callback: function(data, textStatus, xhr) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (item.estatus == 1) {
                ...
                var contenidoDes = JSON.parse(item.descripcion)[validaridioma(idioma)];
                
                    var rex = /(<([^>]+)>)/ig;
                    var replaced=contenidoDes.replace(rex , "");
                    var bien = replaced.substring(0, 100) + '...';
                ...
                Tools.cargarPrincipal({
                    ...
                    callback: function(data, textStatus, xhr) {
                        ...
                        google.maps.event.addListener(marker, 'click', function() {
                            var contenido2 = '<div class="marker"><img src="%logo%" alt="%title%" /><div class="contenedor-pop"><span class="titulo"><a href="%microsite%" class="titulo-pop-map"><strong clas="strong-2">%title%</strong></a></span><div class="tipo %className%"><span class="%logoTipo% %color% size-icon tipoI"></span> %nombreTipo%</div><p class="contenido-mapa"><div class="cont-izq">%contenido%</div></p> </div><a href="%microsite%" class="btn boton-pop %colorBoton%">'+(idioma == 'it' ?  'GUARDA' : 'VIEW') + '</a></div>';
                            var contenido = '<div class="marker"><img src="%logo%" alt="%title%"><div class="contenedor-pop"><span class="titulo"><a href="%microsite%" class="titulo-pop-map"><strong class="strong-3">%title%</strong></a></span><div class="tipo %className%"><span class="%logoTipo% %color% size-icon tipoI"></span> %nombreTipo%</div><p class="contenido-mapa"></p><div class="cont-izq">%contenido%</div><p></p> </div><a href="%microsite%" class="btn boton-pop %colorBoton%">'+(idioma == 'it' ?  'GUARDA' : 'VIEW') + '</a></div>';
                            ...
                            nContenido = nContenido.replace('%contenido%', bien);
                            ...
                            nContenido = nContenido.replace(/%microsite%/gi, '//sanremo-on.com/' + validaridioma(idioma) + '/negozi' +  '/' + JSON.parse(item.url)[validaridioma(idioma)] + '.html');
                            //alert(nContenido);
                        ...
                        contenido = contenido.replace('%href%', '//sanremo-on.com/' + idioma + '/negozi' + /*Tools.clearStr($.trim(helper.getTextTipo(parseInt(item.categoriafk[0].categoriafk))))*/  '/' + JSON.parse(item.url)[validaridioma(idioma)] + '.html');
                        ...
                        contenido = contenido.replace('%categoria%', $.trim(helper.getTextTipo(parseInt(item.categoriafk[0].categoriafk))));
                        console.log($.trim(helper.getTextTipo(parseInt(item.categoriafk[0].categoriafk))));
...
function initialize() {
    ...
    var createMarker = function(params, mapa) {
        ...
        //alert(params.contenido);
        console.log(params.contenido.micro);
		...
		label.bindTo('text', marker, 'position');*/
         ms = params.contenido.micro.split('/');
        console.log(ms[5]);
        var rep = params.contenido.micro.replace(ms[4],'negozi');
        console.log(rep);
        google.maps.event.addListener(marker, 'click', function() {
            ...
            nContenido = nContenido.replace(/%microsite%/gi, rep );//'http://sanremo-on.com/'+idioma+'/negozi/'+ms[5]
            ...
            infowindow.setContent(nContenido);
            //alert(params.contenido);
            //console.log(params.contenido.micro.replace('shopping', 'negozi'));
            infowindow.close();
    ...
    var createLista = function(elemento, lugar) {
        ...
        contenido = contenido.replace('%href%', 'http://sanremo-on.com/'+idioma+'/negozi/'+ms[5]);
...
Tools.cargarPrincipal({
    ...
    callback : function(data, textStatus, xhr){
        ...
        var html = '';
        var idv = '<div class="col-sm-6">';
        var li = '<a href="%link%" style="margin-left:0px; margin-right: 0px;">';
        var qq = '<img src="%src%" style="margin-left:0px; margin-right: 0px;" class="img-responsive" width="100%" height="100%"/>';
        var a = '</a>';
        var vid = '</div>'
        var link='';

        var templateGaleria = '<div class="item" style="width:100%; overflow:hidden;">%pp%</div>';

        var templateGaleria1 = '<div class="item active" style="width:100%; overflow:hidden;">%pp%</div>';
        ...
        var html = '';
        var html2 = '';
    /*  console.log("estos son los"+data);
        console.log(data);*/
        ...
        for (var i = 0,j = 0; i < limite; i = i +2,j++) {
            ...
            var imagenes = ''; 
            var slide = '';
            //imagenes galeria de sponsor Home.
            if(data[i]){
                //alert(data[i].url);
                    if(data[i].url==''){
                        imagenes+=idv+qq.replace(/%src%/gi,Tools.baseAdmin+'public/uploads/banners/'+data[i].imgnormal)+vid;

                    }else{
                        link=li.replace(/%link%/gi,data[i].url);
                        imagenes+=idv+link+qq.replace(/%src%/gi,Tools.baseAdmin+'public/uploads/banners/'+data[i].imgnormal)+a+vid;
                    }
                console.log('en i '+imagenes+'<br>');
            }
            //imagenes+=qq.replace(/%ur%/gi,Tools.baseAdmin+'public/uploads/banners/'+data[i].url);
            

            if(data[i+1]){
                if(data[i+1].url==''){
                    imagenes+=idv+qq.replace(/%src%/gi,Tools.baseAdmin+'public/uploads/banners/'+data[i+1].imgnormal)+vid;

                }else{
                    link=li.replace(/%link%/gi,data[i+1].url);
                    imagenes+=idv+link+qq.replace(/%src%/gi,Tools.baseAdmin+'public/uploads/banners/'+data[i+1].imgnormal)+a+vid;
                }
                //console.log('en i+1 '+imagenes);
...
if($('.texto-cate').val() == 'Tempo libero e benessere'){
    console.log('igual');
}
